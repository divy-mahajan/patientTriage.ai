...
export const PatientDrawer = ({ patient, isOpen, onClose, onSelectForBed, onSelectForDoctor, onRunAssessment }) => {
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-white shadow-2xl border-l border-clinical-border flex flex-col">
          ...
          <div className="border-b border-clinical-border bg-slate-50 px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 text-xs text-clinical-text-secondary font-mono">
                  <span>ID: {patient.patient_id}</span>
                  <span>·</span>
                  <span className="font-bold text-blue-700">
                    {patient.age < 18 ? 'Pediatric' : patient.age >= 65 ? 'Geriatric' : 'Adult'} ({patient.age}y)
                  </span>
                  <span>·</span>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            {/* Vital Signs Grid */}
            <div>
              <label className="text-xs font-bold uppercase tracking-wider text-clinical-text-secondary block mb-1.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg border border-clinical-border p-3">
                ...
                <p className="text-xs font-medium text-red-700">
                  {patient.known_allergies || 'None known'}
                </p>
              </div>
              <div className="rounded-lg border border-clinical-border p-3">
                <span className="text-[10px] uppercase font-bold text-slate-500 block mb-1">
          ...
          <div className="border-t border-clinical-border bg-slate-50 p-4 space-y-2">
            <button
              onClick={() => {
                if (onRunAssessment) onRunAssessment(patient);
                onClose();
              }}
              className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-clinical-primary-container text-white py-2.5 px-4 text-xs font-bold hover:bg-clinical-primary transition shadow-sm cursor-pointer"
            >
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => {
                  if (onSelectForBed) onSelectForBed(patient);
                  onClose();
                }}
                className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-300 bg-white text-slate-700 py-2 px-3 text-xs font-semibold hover:bg-slate-50 transition cursor-pointer"
              >
              ...
              <button
                onClick={() => {
                  if (onSelectForDoctor) onSelectForDoctor(patient);
                  onClose();
                }}
                className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-300 bg-white text-slate-700 py-2 px-3 text-xs font-semibold hover:bg-slate-50 transition cursor-pointer"
              >
